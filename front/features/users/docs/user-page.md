...
## 11. 補足・制約
- ユーザー情報の取得には認証が必要です。認証されていない場合は、ログインページ(/login)に遷移します。
...
このファイルは、ユーザーページの主要な実装です。以下の機能を提供しています:
- ユーザー情報の表示
- ログアウト機能
- 新規投稿作成ボタン

### features/users/mypage/components/User.tsx
このファイルは、ユーザー情報を表示するコンポーネントです。以下の機能を提供しています:
- ユーザーのアイコン画像と名前の表示
- ログアウトボタン
- 新規投稿作成ボタン
- ユーザーの投稿一覧の表示
...
このファイルは、ユーザー情報を取得するAPIを呼び出すための関数を定義しています。

### features/users/mypage/api/fetchUserPosts.ts
このファイルは、ユーザーの投稿一覧を取得するAPIを呼び出すための関数を定義しています。